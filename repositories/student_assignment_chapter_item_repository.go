...
func (studentAssignChapterItemRepo *StudentAssignChapterItemRepository) GetStudentAssignChapterItem (studentAssignChapterItems *models.StudentAssignmentChapterItem,stuId uuid.UUID,chapterId uuid.UUID,itemId int){
	studentAssignChapterItemRepo.DB.Where("stu_id = ? AND chapter_id = ? AND item_id = ?",stuId,chapterId,itemId).Find(studentAssignChapterItems)
}